class Menu {
	...
	irParaSubCategoria(categoria, subcategoria) {
		cy.contains(".left-sidebar", "Category").should("be.visible");
		cy.get(`.left-sidebar ${categoria} a[href^="/category_products/"]`).contains(subcategoria).should("be.visible").click();
		cy.contains(".title", `${categoria.substring(1)} - ${subcategoria} Products`).should("be.visible");
	}

	irParaBrand(brand) {
		cy.contains(".left-sidebar .brands_products", "Brand").should("be.visible");
		cy.get(`.left-sidebar .brands_products a[href*="${brand}"]`).should("be.visible").click();
	}
}
