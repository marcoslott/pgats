class Home {
	...
	validarSeSubscriptionEstaVisivelNoRodape() {
		cy.scrollTo("bottom");
		cy.contains("#footer", "Subscription").scrollIntoView().should("be.visible");
	}
	...
	validarSeItensRecomendadosEstaVisivel() {
		cy.scrollTo("bottom");
		cy.contains(".recommended_items .title", "recommended items").scrollIntoView().should("be.visible");
	}
	...
	validarSeEstaNoTopoDaHome() {
		cy.contains("Full-Fledged practice website for Automation Engineers").scrollIntoView().should("be.visible");
	}
}
