class Produtos {
	...
	validarAListaDeProdutosEAcessarOPrimeiroProduto() {
		cy.get(".single-products").should("have.length.at.least", 1).and("be.visible").first().parent().contains("View Product").click();
	}
			...
			.within(() => {
				cy.get("p")
					...
					.then((text) => {
						...
						else if (indiceProduto == 1) cy.wrap(text.trim()).as("secondProductName");
					});
				cy.get("h2")
					.invoke("text")
					.then((text) => {
	...
	validarReviewEnviadoComSucesso() {
		cy.contains(".alert-success.alert", "Thank you for your review.", { timeout: 10000 }).should("be.visible");
	}
}
